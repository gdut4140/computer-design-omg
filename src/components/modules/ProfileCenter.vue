...
function saveEdit() {
  ...
  target.age = Number(editForm.age) || target.age
  target.gender = editForm.gender
  target.birthday = editForm.birthday || target.birthday
  target.idNumber = editForm.idNumber.trim() || target.idNumber
  ...
  <section class="profile-page">
    ...
    <article class="profile-header-card">
      ...
      <div v-if="currentChild" class="profile-meta">
        ...
        <p class="latest">
          ...
          <b :class="{ safe: currentChild.riskLevel === 'low' }">{{ currentChild.latestRisk }} {{ currentChild.latestRate }}</b>
        </p>
      </div>
    </article>
    ...
    <div class="feature-row">
      <button
        type="button"
        class="feature-btn"
        :class="{ active: activeFeature === 'report' }"
      ...
      <button
        type="button"
        class="feature-btn"
        :class="{ active: activeFeature === 'consult' }"
      ...
      <button
        type="button"
        class="feature-btn"
        :class="{ active: activeFeature === 'basic' }"
      ...
      <div class="report-scroll-wrap">
        <div class="timeline">
          <section v-for="(item, index) in histories" :key="item.id" class="history-item" :class="item.level">
            <div class="axis">
              <div class="dot">{{ item.level === 'high' ? '!' : '✓' }}</div>
              <div v-if="index !== histories.length - 1" class="axis-line"></div>
            </div>
            <article class="item-card">
              <div class="row-head">
                <strong>{{ item.dateText }}</strong>
                <b class="tag" :class="item.level === 'high' ? 'danger' : 'safe'">{{ item.riskText }}</b>
              </div>
              <p class="time">{{ item.timeText }} · {{ item.modeText }}</p>
              <p class="value" :class="item.level === 'high' ? 'danger-text' : 'safe-text'">{{ item.rateText }}</p>
            </article>
          </section>
        </div>
      </div>
    ...
    <el-dialog
      ...
      modal-class="profile-edit-overlay"
      title="修改患儿信息"
      width="500px"
      append-to-body
    >
...
.child-card {
  ...
  border: 1px solid rgba(73, 97, 151, 0.36);
  background:
    radial-gradient(circle at 102% -8%, rgba(35, 220, 210, 0.2), transparent 30%),
    linear-gradient(165deg, rgba(28, 41, 72, 0.95), rgba(20, 30, 56, 0.9));
  padding: 16px;
  cursor: pointer;
  ...
  &::before {
    ...
    bottom: 0;
    width: 3px;
    background: linear-gradient(180deg, rgba(42, 225, 214, 0.9), rgba(20, 191, 183, 0.35));
    opacity: 0.55;
  }

  &:hover {
    border-color: rgba(32, 218, 209, 0.52);
    transform: translateY(-2px);
    box-shadow: 0 14px 30px rgba(5, 13, 31, 0.36);
  }

  &.active {
    border-color: rgba(32, 218, 209, 0.75);
    box-shadow:
      0 0 0 1px rgba(32, 218, 209, 0.24) inset,
